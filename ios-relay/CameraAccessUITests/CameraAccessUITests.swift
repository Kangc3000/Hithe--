...
final class CameraAccessUITests: XCTestCase {
  ...
  /// Taps "Connect Meta glasses" to trigger registration via the fake handler,
  ...
  private func registerViaUI() {
    let connectButton = app.buttons["Connect Meta glasses"]
  ...
  /// Waits for the "Start live video" button to exist and be enabled (mock device active).
  @discardableResult
  private func waitForStartStreamingEnabled(timeout: TimeInterval = 15) -> XCUIElement {
    let startButton = app.buttons["Start live video"]
    XCTAssertTrue(startButton.waitForExistence(timeout: timeout), "Start live video button should appear")
  ...
  /// Waits for the "Start live video" button to exist and be disabled (device inactive).
  @discardableResult
  ...
  func testLaunchWithoutDeviceShowsHomeScreen() {
    ...
    XCTAssertTrue(
      ...
      "HomeScreenView should show 'Connect Meta glasses' when no device is paired"
    )
  }
  ...
  func testStartAndStopStreaming() {
    ...
    XCTAssertTrue(app.staticTexts["Glasses ready"].exists, "NonStreamView title should reappear")
  }
